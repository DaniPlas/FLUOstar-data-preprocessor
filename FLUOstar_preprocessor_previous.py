...
def get_norm_times(df):
    ...
    for t in range(1, len(df.columns)):
        ini_time = int(input('Enter initial time for normalization of column ' + str(df.columns[t]) + " "))
        final_time = int(input('Enter final time for normalization of column ' + str(df.columns[t]) + " "))
